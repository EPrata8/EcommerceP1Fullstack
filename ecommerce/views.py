  ...
  if str_id in carrinho:
    ...
    if produto.estoque > 0:
      carrinho[str_id] = {
          ...
          'quantidade': 1,
          'organizacao_id': produto.organizacao.id,  # <-- Salvando a organização para vincular ao cupom correto!
      }
  ...
  if codigo_cupom:
    try:
      ...
      for item in produtos_no_carrinho:
        # O desconto só se aplica se o produto pertencer EXATAMENTE à mesma organização do cupom!
        if (
